...
//  线性表
//
//  Created by MacBook Pro on 2020/10/12.
//
#define ERROR -1
#include<iostream>
#include "List.h"
...
public:
    ...
    {
        if(listSize < maxSize)// "List capacity exceeded"
    ...
    E remove()
    {
        if((curr>=0) && (curr < listSize))// "No element
        {
    ...
    {
        if((pos>=0)&&(pos<=listSize))// "Pos out of range"
        curr = pos;
    }
